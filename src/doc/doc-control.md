...
::: warning 注意
升级至Matrix v2.3.0后，可以直接点击Matrix输入法下方灰色区域快速跳转到系统输入法设置页进行输入法切换。
:::
...
安卓投屏触控默认使用 [sonic-android-apk](https://felixyang007.github.io/matrixUserGuide/saa) 实现实时触控，解决了minitouch在安卓10以上无法实时使用的场景。但是部分特殊情况下，当sonic-android-apk不可用时，可以点击投屏区域右侧工具栏的【修复触控】切换为adb原生触控。
...
当默认初始化失败后，需要调整对应的设置，可以前往 [这里](https://felixyang007.github.io/matrixUserGuide/deploy/android-deploy.html) 检查配置。然后点击【初始化UIAutomator2Server】直至Status为Connected为止。

### 远程音频

远程音频功能目前依赖 [sonic-android-apk](https://felixyang007.github.io/matrixUserGuide/saa/re-saa.html) 实现实时音频传输，开启后可以直接在网页上播放远程设备的实时音频。
